(function () {
    ...
    function renderPreview(container, colors) {
        var labels = cfg().labels || {};
        container.innerHTML = '';
        ...
        card.appendChild(el('h3', { style: 'color:' + colors.heading_h1_color }, labels.sample || 'Sample heading'));
        var p = el('p', { style: 'color:' + colors.content_text_color });
        ...
        card.appendChild(p);
        card.appendChild(el('span', { className: 'poetheme-studio__pv-cta', style: 'background:' + colors.cta_background_color + ';color:' + colors.cta_text_color }, labels.cta || 'Button'));
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function update() {
            var s = seeds();
            current = generate(s);
            renderSwatches(swatches, current.meta);
            renderContrast(contrastBox, current.colors);
            renderPreview(preview, current.colors);
        ...
        harmony.addEventListener('change', update);
        mode.addEventListener('change', update);
        accentButtons.addEventListener('change', update);
        nameInput.addEventListener('input', function () {
            payload.value = JSON.stringify({ name: nameInput.value, seeds: seeds(), colors: current.colors });
        });
